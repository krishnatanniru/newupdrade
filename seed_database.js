 ...
 * 
 * This script recreates all tables and seeds initial data
 * after the database was deleted.
 */
...
// SHA-256 hash function (same as the app uses)
...
const BRANCHES = [
  { 
    id: 'b1', 
    name: 'Speed Fitness Mumbai Central', 
    address: 'Plot 45, BKC, Mumbai, Maharashtra 400051', 
    phone: '+91 98765 43210', 
    email: 'mumbai@ironflow.in',
    gstin: '27AABCU1234F1Z1',
    gstPercentage: 18,
    equipment: 'Cardio: 5 Treadmills, 2 Spin Bikes. Strength: Full Smith Machine, Dumbbells up to 40kg.'
  },
  { 
    id: 'b2', 
    name: 'Speed Fitness Bangalore East', 
    address: 'Indiranagar 100ft Rd, Bengaluru, Karnataka 560038', 
    phone: '+91 98765 01234', 
    email: 'bangalore@ironflow.in',
    gstin: '29AABCU5678F1Z2',
    gstPercentage: 18,
    equipment: 'Functional Training Zone: Kettlebells, Battle Ropes, TRX, Medicine Balls.'
  },
];
...
async function seed() {
  ...
  if (bErr) {
    ...
    console.log('\n⚠️  It looks like the tables do not exist yet.');
    console.log('   Please run the SQL in complete_supabase_setup_updated.sql');
    console.log('   in your Supabase Dashboard > SQL Editor first, then re-run this script.\n');
    process.exit(1);
  } else {
  ...
  if (pErr) {
    console.error('   ❌ Plans error:', pErr.message);
  } else {
  ...
  if (oErr) {
    console.error('   ❌ Offers error:', oErr.message);
  } else {
